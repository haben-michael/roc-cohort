...
## xi <- subj.fit(hiv.data,hiv.params$rho,hiv.params$sigma,eta,t=t)
## hiv.roc.3 <- roc(hiv.data,xi,rho=hiv.params$rho,sigma=hiv.params$sigma,t)
## ## plot(hiv.roc.3)
## t <- 5
## hiv.roc.3 <- roc.3(hiv.data,rho=hiv.params$rho,sigma=hiv.params$sigma,ts=t)
## ## plot(hiv.roc.3)
## bootstrap.reps <- 5e1
## hiv.roc.3.bs <- replicate(bootstrap.reps,{
##     ## cat('.')
##     tryCatch({
##         hiv.data.star <- hiv.data[sample(1:N,replace=TRUE)]
##         roc.3(hiv.data.star,rho=hiv.params$rho,sigma=hiv.params$sigma,ts=t)
##     },error=function(e)NA)
##     ## hiv.data.star <- hiv.data[sample(1:N,replace=TRUE)]
##     ## eta <- pop.fit(hiv.data.star,rho=hiv.params$rho,t=t)
##     ## print(eta)
##     ## xi <- subj.fit(hiv.data.star,hiv.params$rho,hiv.params$sigma,eta,t=t)
...
t <- 10
eta <- pop.fit(hiv.data,rho=hiv.params$rho,t=t-1)
xi <- subj.fit(hiv.data,hiv.params$rho,hiv.params$sigma,eta,t=t-1)
hiv.roc.3 <- roc(hiv.data,xi,rho=hiv.params$rho,sigma=hiv.params$sigma,t)
plot(hiv.roc.3)
bootstrap.reps <-3e2
hiv.roc.3.bs <- replicate(bootstrap.reps,{
    cat('.')
    hiv.data.star <- hiv.data[sample(1:N,replace=TRUE)]
    eta.star <- pop.fit(hiv.data.star,rho=hiv.params$rho,t=t+1)
    ## print(eta)                        
    xi <- subj.fit(hiv.data.star,hiv.params$rho,hiv.params$sigma,eta=eta.star,t=t)
    tryCatch(roc(hiv.data.star,xi,rho=hiv.params$rho,sigma=hiv.params$sigma,t),error=function(e)NA)

},simplify=F)
hiv.roc.3.bs <- hiv.roc.3.bs[!is.na(hiv.roc.3.bs)]
bootstrap.reps - length(hiv.roc.3.bs)

plot(hiv.roc.3,bootstrap.rocs=hiv.roc.3.bs,alpha=.05,N=N,gaussian.CI=TRUE)
